...
On `main`, the WebAssembly build carries no intrinsics of its own: under Emscripten every ISA predicate misses and the scalar path is autovectorised by `-msimd128` (`CMakeLists.txt:330`). The live page and `main` diverge: `getglyph.vercel.app` serves a real SIMD build off [`yadava5/fix-hero-media-validation`](https://github.com/yadava5/glyph/tree/yadava5/fix-hero-media-validation) — `/wasm/fast_mnist.wasm`, 46,960 bytes, `application/wasm`. `curl -s https://getglyph.vercel.app/wasm/fast_mnist.wasm | shasum -a 256` gives `396dc0f4848ddc0b9c35420cdad5e5cc0af53f4a408024ba10f7364470d9807e` — byte-identical to that branch's blob.
